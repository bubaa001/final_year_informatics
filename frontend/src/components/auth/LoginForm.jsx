...
const LoginForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      const userRole = JSON.parse(atob(localStorage.getItem('access_token').split('.')[1])).role;
  ...
  return (
    <div className="auth-bg">
      ...
      >
        ...
        {/* Error */}
        {error && (
          <div className="glass-error stagger-children" style={{ marginBottom: '20px' }}>
            <div>
              {typeof error === 'string' ? error : error.email || error.password || 'Login failed'}
            </div>
          </div>
        )}
        ...
        {/* Divider */}
        <div className="glass-divider" style={{ margin: '24px 0' }}>
        ...
        >
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.9rem' }}>
            ...
            <Link to="/register" className="glass-link" style={{ fontWeight: 600 }}>
              Sign up
            </Link>
          </p>
        </div>
